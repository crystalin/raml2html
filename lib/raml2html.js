...
renderer.list = function(body, ordered) {
  ...
  return '<' + type + ' class="marked-collection">\n' + body + '</' + type + '>\n';
};

renderer.listitem = function(text) {
  return '<li class="marked-item">' + text + '</li>\n';
};
...
function _ifEquals(v1, v2, options) {
    if (v1 == v2) {
        return options.fn(this)
    } else {
        return options.inverse(this);
...
function _ifDiff(v1, v2, options) {
    if (v1 != v2) {
        return options.fn(this)
    } else {
        return options.inverse(this);
...
function _section(v1, options) {
    section = v1;
    return options.fn(this);
}

function _ifSection(v1, options) {
    if (section == v1) {
        return options.fn(this);
    } else {
        return options.inverse(this);
...
function _ramlObj(options) {
    return options.fn(ramlObject);
}
...
function _securedSchema(options) {
    if (!this.securedBy) {
        return ;
    }
    var result = "";
    for (var i = 0; i < this.securedBy.length; i++) {
        for (var j = 0; j < ramlObject.securitySchemes.length; j++) {
            var schema = ramlObject.securitySchemes[j][this.securedBy[i]];
            if (schema) {
                result += options.fn(schema.describedBy);
            }
        }
    }
    return result;
...
function _replaceUriParameters(parentUri, relativeUri) {
    var uri = parentUri + relativeUri;

    for (var i = this.allUriParameters.length - 1; i >= 0; i--) {
        var uriParameter = this.allUriParameters[i];
        uri = uri.replace('{'+uriParameter.displayName+'}', uriParameter.example);
    };
    return uri;
}
...
function getDefaultConfig(https, mainTemplate, resourceTemplate, itemTemplate) {
    ...
    return {
        ...
        helpers: {
            ...
            ifEquals: _ifEquals,
            ifDiff: _ifDiff,
            ifSection: _ifSection,
            section: _section,
            ramlObj: _ramlObj,
            toLowerCase: _toLowerCase,
            toUpperCase: _toUpperCase,
            securedSchema: _securedSchema,
            replaceUriParameters: _replaceUriParameters
        },
        partials: {
